(function (factory) {
    ...
    const redstone_api_1 = tslib_1.__importDefault(require("redstone-api"));
    const utils_1 = tslib_1.__importDefault(require("./utils"));
    class Community {
        cacheServer = 'https://cache.community.xyz/';
        contractSrc = 'ngMml4jmlxu0umpiQCsHgPX2pb_Yz6YDB8f7G6j-tpI';
        mainContract = 'mzvUgNc8YFk0w5K5H7c8pyT-FC5Y_ba0r7_8766Kx74';
        txFeeUsd = 0.5;
        createFeeUsd = 3;
        createFee = 0.83;
        txFee = 0.21;
        ...
        state;
        firstCall = true;
        cacheRefreshInterval = 1000 * 60 * 2; // 2 minutes
        stateCallInProgress = false;
        warnAfter = 60 * 60 * 24 * 1000; // 24 hours
        updatedFees = false;
        /**
         ...
         */
        async setState(name, ticker, balances, quorum = 50, support = 50, voteLength = 2000, lockMinLength = 720, lockMaxLength = 10000, vault = {}, votes = [], roles = {}, extraSettings = []) {
        ...
        async create(tags = []) {
            ...
            const communityID = await smartweave_1.createContractFromTx(this.arweave, this.wallet, this.contractSrc, JSON.stringify(toSubmit), tags, target, winstonQty);
         ...
         */
        async getCreateCost(inAr = false, options) {
            if (!this.updatedFees) {
         ...
         * @param options - If return inAr is set to true, these options are used to format the returned AR value.
         */
        async getActionCost(inAr = false, options) {
            if (!this.updatedFees) {
                await new Promise((resolve) => setTimeout(() => resolve(true), 100));
         ...
         */
        async get(params = { function: 'balance' }) {
         ...
         */
        async getBalance(target = this.walletAddress) {
         ...
         * @returns Current target token balance
         */
        async getUnlockedBalance(target = this.walletAddress) {
         ...
         * @returns Current target token balance
         */
        async getVaultBalance(target = this.walletAddress) {
         ...
         */
        async getRole(target = this.walletAddress) {
         ...
         */
        async selectWeightedHolder(balances = this.state.balances, vault = this.state.vault) {
            if (!this.state) {
        ...
        async getFees() {
            try {
                const price = await redstone_api_1.default.getPrice('AR');
         ...
         */
        async transfer(target, qty, tags = []) {
            tags = [
                ...(await this.cleanTags(tags)),
                ...[
         ...
         * @returns The transaction ID for this action
         */
        async transferLocked(target, qty, lockLength, tags = []) {
            tags = [
                ...(await this.cleanTags(tags)),
                ...[
         ...
         * @returns The transaction ID for this action
         */
        async lockBalance(qty, lockLength, tags = []) {
            tags = [
                ...(await this.cleanTags(tags)),
                ...[
         ...
         * @returns The transaction ID for this action
         */
        async increaseVault(vaultId, lockLength, tags = []) {
            tags = [
                ...(await this.cleanTags(tags)),
                ...[
        ...
        async proposeVote(params, tags = []) {
            ...
            const input = { ...pCopy, function: 'propose' };
            tags = [
                ...(await this.cleanTags(tags)),
                ...[
         ...
         */
        async chargeFee(fee = this.txFee) {
            ...
            let state;
            try {
                state = (await axios_1.default(`${this.cacheServer}contract/${this.mainContract}`)).data;
            }
            catch (e) {
                state = await smartweave_1.readContract(this.arweave, this.mainContract);
            }
        ...
        async update(recall = false) {
            ...
            let state;
            try {
                state = (await axios_1.default(`${this.cacheServer}contract/${this.communityContract}`)).data;
            }
            catch (e) {
                state = await smartweave_1.readContract(this.arweave, this.communityContract);
            }
         ...
         */
        async interact(input, tags, fee = this.txFee) {
            const { target, winstonQty } = await this.chargeFee(fee);
            tags.push({ name: 'Type', value: 'ArweaveActivity' });
            const res = await smartweave_1.interactWriteDryRun(this.arweave, this.wallet || 'use_wallet', this.communityContract, input, tags, target, winstonQty);
            if (res.type === 'error') {
                //  || res.type === 'exception'
                throw new Error(res.result);
            }
            return smartweave_1.interactWrite(this.arweave, this.wallet || 'use_wallet', this.communityContract, input, tags, target, winstonQty);
        }
        /**
